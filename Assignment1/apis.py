from sqlalch import db,User,Ride
...
app.config['SQLALCHEMY_DATABASE_URI'] 	= 'sqlite:///F:\\Notes\\Semester 6\\Cloud Computing\\Assignment 1\\test7.db'
db.init_app(app)
	...
	if(isinstance(res,list)):
		for dic in res:
			...
			for s in dic["users"][1:-1].split(", "):
				if(not(s==name or (s[0]==" "and s[1:]==name))):
					users_list.append(s.strip())
			print(users_list)
...
def get_ride_details():
	...
	if(response.text=="{}\n"):
		return jsonify({}),204
	if(response.text[0]=="["):
		return response.text.replace("created_by","username"),200
	else:
		return "[\n"+response.text.replace("created_by","username")+"]",200
...
def join_ride(rideid):
	...
	if(username in users_string):
		return jsonify({}),400
	users_list.append(username.strip())
