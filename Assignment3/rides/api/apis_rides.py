...
'''
'''def get_count():
	resp_data = {'table':"ride",'columns':["rideId","created_by","timestamp"],"where":"None","delete":"False"}
	response = requests.post("http://ec2-54-208-250-221.compute-1.amazonaws.com/api/v1/db/read",json=resp_data)
...
def new_ride():
	...
	#if(response.text=="{}\n"):
	#	return jsonify({}),400
	response = requests.get("http://CCProject-419813030.us-east-1.elb.amazonaws.com/api/v1/users",headers={'Origin':'ec2-54-208-250-221.compute-1.amazonaws.com'})
	li_users = json.loads(response.text)
	flag = 0
	for name in li_users:
		if(name==created_by):
			flag=1
			break
	if(flag==0):
	...
	response = requests.post("http://ec2-54-208-250-221.compute-1.amazonaws.com/api/v1/db/write",json=resp_data)
	#rides_count += 1
...
def ride_details(rideid):
    ...
    response = requests.post(
        "http://ec2-54-208-250-221.compute-1.amazonaws.com/api/v1/db/read", json=resp_data)
    if(response.text == "{}\n"):
        return jsonify({}), 204
...
def join_ride(rideid):
	...
	response1 = requests.post("http://ec2-54-208-250-221.compute-1.amazonaws.com/api/v1/db/read", json=resp_data)
	if(response1.text == "{}\n"):
		return jsonify({}), 400
	req_data = request.get_json()
...
def return_405_single_ride(rideid):
	global http_count
	http_count += 1
	write_pickle()
	return jsonify({}),405
...
'''@app.route('/api/v1/db/clear',methods=['PUT','GET','PATCH','DELETE','COPY','HEAD','OPTIONS','LINK','UNLINK','PURGE','LOCK','UNLOCK','PROPFIND','VIEW'])
def return_405_clear():
	global http_count
	http_count += 1
	write_pickle()
	return jsonify({}),405'''
...
def get_rides_count():
	...
	if(isinstance(dic,list)):
		...
		rides_count= 1
	return jsonify([rides_count]),200

@app.route('/api/v1/rides/count', methods=['POST','PATCH','DELETE','COPY','HEAD','OPTIONS','LINK','UNLINK','PURGE','LOCK','UNLOCK','PROPFIND','VIEW','PUT'])
def return_405_ridescount():
	global http_count
	http_count += 1
	write_pickle()
	return jsonify({}),405
