	...
	while(True):
		...
		if(is_first_time == 1):
			for container in client.containers.list():
				if(str(container.image) == "<Image: 'test_slave:latest'>" and not(is_master(container))):
				...
				elif(str(container.image) == "<Image: 'test_master:latest'>"):
			...
			slave_names = [str(container.name) for container in client.containers.list() if str(container.image) == "<Image: 'test_slave:latest'>" and not(is_master(container))]
			for zk_name in zk.get_children("/workers/slaves"):
				if zk_name not in slave_names:
					if(zk.exists("/workers/slaves/"+str(zk_name))):
						...
						container = client.containers.run("test_slave:latest", network = "test_default", environment =["IS_MASTER=False","IS_FIRST_SLAVE=False"], detach = True)
						zk.create("/workers/slaves/"+str(container.name), str(container.id).encode('utf-8'))
			...
			for zk_name in zk.get_children("/workers/master"):
				if zk_name not in master_name:
					if(zk.exists("/workers/master/"+str(zk_name))):
						...
						zk.create("/workers/master/"+str(min_container.name), str(min_container.id).encode('utf-8'))
						print("ZOOKEEPER ELECTED NEW MASTER : ",str(min_container.name))
						container = client.containers.run("test_slave:latest", network = "test_default", environment =["IS_MASTER=False","IS_FIRST_SLAVE=False"], detach = True)
						zk.create("/workers/slaves/"+str(container.name), str(container.id).encode('utf-8'))
						print("ZOOKEEPER CREATED SLAVE "+container.name)
...
def deploy_slaves():
	...
	if(curr_num_containers > ((number_of_reads-1)//20)+1):
		for container in client.containers.list():
			if(str(container.image) == "<Image: 'test_slave:latest'>" and not(is_master(container)) and curr_num_containers>((number_of_reads-1)//20)+1 and not(curr_num_containers==1)):
